...
- `q2_model.py`：完整求解入口，负责数据读取、条件价格响应估计、需求与成本预测、联合场景生成、周级定价补货优化及结果导出；
...
优化采用两阶段结构：逐日 Differential Evolution 与局部 SLSQP 启发式产生满足历史区间和日变动约束的候选初值；随后对七天共 84 个加价率与补货量变量进行周级 SLSQP 联合精修。差分进化仅用于初值搜索，其成功标志不作为最终方案的全局最优证明。周级目标的梯度由各场景利润的分段解析次梯度计算，分别处理库存/需求约束状态以及商誉成本中正毛利/非正毛利状态。主方案后继续计算 8 组完整敏感性策略。
